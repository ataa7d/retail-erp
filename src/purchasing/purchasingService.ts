...
export async function createPurchaseOrder(client: Client, p: CreatePurchaseOrderParams): Promise<string> {
  ...
  const header = await client.query<{ id: string }>(
    `INSERT INTO purchase_orders (company_id, store_id, supplier_id, document_number, order_date, expected_date, fiscal_period_id, created_by)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`,
    [p.companyId, p.storeId, p.supplierId, documentNumber, p.orderDate, p.expectedDate ?? null, p.fiscalPeriodId, p.createdBy ?? null],
  );
...
export async function createGoodsReceipt(client: Client, p: CreateGoodsReceiptParams): Promise<string> {
  ...
  const header = await client.query<{ id: string }>(
    `INSERT INTO goods_receipts (company_id, store_id, purchase_order_id, supplier_id, document_number, receipt_date, fiscal_period_id, created_by)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`,
    [p.companyId, p.storeId, p.purchaseOrderId, p.supplierId, documentNumber, p.receiptDate, p.fiscalPeriodId, p.createdBy ?? null],
  );
    ...
    await client.query(
      `INSERT INTO goods_receipt_lines
         (company_id, goods_receipt_id, line_number, purchase_order_line_id, item_variant_id,
          qty_received, base_unit_cost, unit_cost)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $7)`, // unit_cost starts equal to base_unit_cost; landed cost updates it at posting
      [p.companyId, receiptId, lineNumber, line.purchaseOrderLineId, line.itemVariantId, line.qtyReceived, baseUnitCost],
    );
  }
...
export async function postGoodsReceipt(client: Client, receiptId: string, postedBy: string): Promise<void> {
  ...
  const lines = await client.query<{ id: string; item_variant_id: string; qty_received: string; base_unit_cost: string }>(
    `SELECT id, item_variant_id, qty_received, base_unit_cost FROM goods_receipt_lines WHERE goods_receipt_id = $1 ORDER BY line_number`,
    [receiptId],
  );
  ...
  if (charges.rows.length > 0) {
    ...
    for (const line of lines.rows) {
      ...
      weights.set(line.id, {
        value: Number(line.qty_received) * Number(line.base_unit_cost),
  ...
  for (const line of lines.rows) {
    ...
    const baseUnitCost = Number(line.base_unit_cost);
    const unitCost = round2(baseUnitCost + landedCostAmount / qtyReceived);
  ...
  const totalMerchandiseValue = round2(
    lines.rows.reduce((s, l) => s + Number(l.qty_received) * Number(l.base_unit_cost), 0),
  );
...
export async function createSupplierInvoice(client: Client, p: CreateSupplierInvoiceParams): Promise<string> {
  ...
  const header = await client.query<{ id: string }>(
    `INSERT INTO supplier_invoices
       (company_id, supplier_id, purchase_order_id, supplier_invoice_number, document_number, invoice_date, fiscal_period_id, created_by)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`,
    [p.companyId, p.supplierId, p.purchaseOrderId, p.supplierInvoiceNumber, documentNumber, p.invoiceDate, p.fiscalPeriodId, p.createdBy ?? null],
  );
...
export async function postSupplierInvoice(client: Client, invoiceId: string, postedBy: string): Promise<void> {
  const invoiceResult = await client.query(
    `SELECT company_id, invoice_date, fiscal_period_id, net_amount, vat_amount, gross_amount FROM supplier_invoices WHERE id = $1`,
    [invoiceId],
  );
  ...
  let totalVariance = 0; // positive = supplier charged more (debit variance), negative = less (credit variance)
  for (const line of lines.rows) {
    const grLine = await client.query<{ base_unit_cost: string }>(
      `SELECT base_unit_cost FROM goods_receipt_lines WHERE id = $1`,
      [line.goods_receipt_line_id],
    );
    const baseUnitCost = Number(grLine.rows[0]!.base_unit_cost);
    const qty = Number(line.qty);
    totalGrniClear = round2(totalGrniClear + qty * baseUnitCost);
    totalVariance = round2(totalVariance + (Number(line.net_amount) - qty * baseUnitCost));
  }
  ...
  await client.query(
    `INSERT INTO journal_lines (company_id, journal_id, line_number, account_id, credit_amount, description)
     VALUES ($1, $2, $3, $4, $5, 'accounts payable')`,
    [invoice.company_id, journalId, lineNumber, apAccountId, invoice.gross_amount],
  );

  await client.query(`UPDATE journals SET document_status = 'posted' WHERE id = $1`, [journalId]);
  await client.query(
    `UPDATE supplier_invoices SET document_status = 'posted', posted_by = $2, journal_id = $3 WHERE id = $1`,
...
export async function postSupplierCreditNote(client: Client, creditNoteId: string, postedBy: string): Promise<void> {
  const cnResult = await client.query(
    `SELECT company_id, store_id, credit_note_date, fiscal_period_id, net_amount, vat_amount, gross_amount
